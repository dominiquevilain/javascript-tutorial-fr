...
```js run
let userName = prompt("Votre prénom ?", "Alice");
let isTeaWanted = confirm("Voulez-vous une tasse de thé ?");

alert( "Visiteur : " + userName ); // Alice
alert( "Thé souhaité " + isTeaWanted ); // true
```
...
switch (age) {
  case 18:
    alert("Ne fonctionnera pas"); // le résultat de prompt est une chaîne de caractères, pas un nombre
  ...
  default:
    alert("Toute valeur différente de celle indiquée ci-dessus");
}
```
